// @Title  Likes
// @Description  该文件提供关于Likes行为的各种方法
// @Author  MGAronya（张健）
...
func (l Likes) PublishBehavior(score float64, userId uuid.UUID) error {
	...
	for i := range badgeBehaviors {
		...
		if err := l.DB.Where("id = ?", badgeBehaviors[i].BadgeId).First(&badge).Error; err != nil {
			log.Println(err)
			continue
		}
		var userBadge model.UserBadge
		if err := l.DB.Where("badge_id = ?", badgeBehaviors[i].BadgeId).First(&userBadge).Error; err != nil {
			log.Println(err)
			continue
		}
		...
		if userBadge.MaxScore < score {
			...
			if new != old {
				...
				l.Redis.Publish(l.ctx, "BadgePublish"+userId.String(), v)
			}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (l Likes) Description() string {
	return "用户收到点赞总数"
}
...
func NewLikes() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return Likes{DB: db, Redis: redis, ctx: ctx}
}
