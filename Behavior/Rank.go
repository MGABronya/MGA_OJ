// @Title  Rank
// @Description  该文件提供关于Rank行为的各种方法
// @Author  MGAronya
...
// @title    UserBehavior
// @description   查看用户竞赛分排名百分比（超过百分之多少）
// @auth      MGAronya       2022-9-16 12:15
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (r Rank) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if r.DB.Where("name = ? and user_id = ?", "Rank", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (r Rank) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if r.DB.Where("name = ? and user_id = ?", "Rank", userId).First(&behavior).Error != nil {
		behavior.Name = "Rank"
		...
		behavior.Score = 0
		r.DB.Create(&behavior)
	}
	behavior.Score = score
	// TODO 更新值
	r.DB.Where("name = ? and user_id = ?", "Rank", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	r.DB.Where("name = ?", "Rank").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
	for i := range badgeBehaviors {
		var badge model.Badge
		if err := r.DB.Where("id = ?", badgeBehaviors[i].BadgeId).First(&badge).Error; err != nil {
			log.Println(err)
			continue
		}
		var userBadge model.UserBadge
		if err := r.DB.Where("badge_id = ?", badgeBehaviors[i].BadgeId).First(&userBadge).Error; err != nil {
			log.Println(err)
			continue
		}
		...
		if userBadge.MaxScore < score {
			...
			if new != old {
				...
				r.Redis.Publish(r.ctx, "BadgePublish"+userId.String(), v)
			}
...
func NewRank() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return Rank{DB: db, Redis: redis, ctx: ctx}
}
