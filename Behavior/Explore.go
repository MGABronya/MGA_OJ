// @Title  Explore
// @Description  该文件提供关于Explore行为的各种方法
// @Author  MGAronya
...
// @return   void
func (a Explore) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "Explore", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Explore) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "Explore", userId).First(&behavior).Error != nil {
		behavior.Name = "Explore"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "Explore", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "Explore").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Explore) Description() string {
	return "用户为题目打上标签的次数"
}
...
func NewExplore() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return Explore{DB: db, Redis: redis, ctx: ctx}
}
