...
// @Description  该文件提供关于DynamicProgramming行为的各种方法
// @Author  MGAronya
...
// DynamicProgramming			定义了DynamicProgramming行为类
...
// @return   void
func (a DynamicProgramming) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "DynamicProgramming", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a DynamicProgramming) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "DynamicProgramming", userId).First(&behavior).Error != nil {
		behavior.Name = "DynamicProgramming"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "DynamicProgramming", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "DynamicProgramming").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a DynamicProgramming) Description() string {
	return "用户通过的动态规划的题目数量"
}

// @title    NewDynamicProgramming
// @description   新建一个BeahviorInterface
// @auth      MGAronya       2022-9-16 12:23
// @param    void
// @return   BeahviorInterface 		返回一个BeahviorInterface用于调用各种函数
func NewDynamicProgramming() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return DynamicProgramming{DB: db, Redis: redis, ctx: ctx}
}
