// @Title  DataStructure
// @Description  该文件提供关于DataStructure行为的各种方法
// @Author  MGAronya
...
// DataStructure			定义了DataStructure行为类
...
// @return   void
func (a DataStructure) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "DataStructure", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a DataStructure) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "DataStructure", userId).First(&behavior).Error != nil {
		behavior.Name = "DataStructure"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "DataStructure", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "DataStructure").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a DataStructure) Description() string {
	return "用户通过的数据结构的题目数量"
}

// @title    NewDataStructure
// @description   新建一个BeahviorInterface
// @auth      MGAronya       2022-9-16 12:23
// @param    void
// @return   BeahviorInterface 		返回一个BeahviorInterface用于调用各种函数
func NewDataStructure() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return DataStructure{DB: db, Redis: redis, ctx: ctx}
}
