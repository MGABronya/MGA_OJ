// @Title  Collects
// @Description  该文件提供关于Collects行为的各种方法
// @Author  MGAronya（张健）
...
// @title    UserBehavior
// @description   查看用户被收藏次数
// @auth      MGAronya（张健）       2022-9-16 12:15
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (c Collects) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if c.DB.Where("name = ? and user_id = ?", "Collects", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (c Collects) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if c.DB.Where("name = ? and user_id = ?", "Collects", userId).First(&behavior).Error != nil {
		behavior.Name = "Collects"
		behavior.UserId = userId
		behavior.Score = 0
		c.DB.Create(&behavior)
	}
	behavior.Score += score
	// TODO 更新值
	c.DB.Where("name = ? and user_id = ?", "Collects", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	c.DB.Where("name = ?", "Collects").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
	for i := range badgeBehaviors {
		var badge model.Badge
		if err := c.DB.Where("id = ?", badgeBehaviors[i].BadgeId).First(&badge).Error; err != nil {
			log.Println(err)
			continue
		}
		var userBadge model.UserBadge
		if err := c.DB.Where("badge_id = ?", badgeBehaviors[i].BadgeId).First(&userBadge).Error; err != nil {
			log.Println(err)
			continue
		}
		...
		if userBadge.MaxScore < score {
			...
			if new != old {
				...
				c.Redis.Publish(c.ctx, "BadgePublish"+userId.String(), v)
			}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (c Collects) Description() string {
	return "用户收到收藏总数"
}
...
func NewCollects() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return Collects{DB: db, Redis: redis, ctx: ctx}
}
