...
// @return   void
func (a AK) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "AK", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a AK) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "AK", userId).First(&behavior).Error != nil {
		behavior.Name = "AK"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "AK", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "AK").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
func NewAK() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return AK{DB: db, Redis: redis, ctx: ctx}
}
