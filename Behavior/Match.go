// @Title  Match
// @Description  该文件提供关于Match行为的各种方法
// @Author  MGAronya
...
// @return   void
func (a Match) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "Match", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Match) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "Match", userId).First(&behavior).Error != nil {
		behavior.Name = "Match"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "Match", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "Match").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Match) Description() string {
	return "用户最近一次在匹配赛中的参赛人数/参赛排名"
}
...
func NewMatch() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return Match{DB: db, Redis: redis, ctx: ctx}
}
