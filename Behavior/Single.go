// @Title  Single
// @Description  该文件提供关于Single行为的各种方法
// @Author  MGAronya
...
// @return   void
func (a Single) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "Single", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Single) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "Single", userId).First(&behavior).Error != nil {
		behavior.Name = "Single"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "Single", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "Single").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Single) Description() string {
	return "用户最近一次在个人赛中的参赛人数/参赛排名"
}
...
func NewSingle() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return Single{DB: db, Redis: redis, ctx: ctx}
}
