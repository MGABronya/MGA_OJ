...
// @return   void
func (a OI) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "OI", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a OI) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "OI", userId).First(&behavior).Error != nil {
		behavior.Name = "OI"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "OI", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "OI").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a OI) Description() string {
	return "用户最近一次在OI赛中的参赛人数/参赛排名"
}
...
func NewOI() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return OI{DB: db, Redis: redis, ctx: ctx}
}
