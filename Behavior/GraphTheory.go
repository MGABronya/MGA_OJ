// @Title  GraphTheory
// @Description  该文件提供关于GraphTheory行为的各种方法
// @Author  MGAronya
...
// @return   void
func (a GraphTheory) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "GraphTheory", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a GraphTheory) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "GraphTheory", userId).First(&behavior).Error != nil {
		behavior.Name = "GraphTheory"
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "GraphTheory", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "GraphTheory").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a GraphTheory) Description() string {
	return "用户通过的图论的题目数量"
}

// @title    NewGraphTheory
// @description   新建一个BeahviorInterface
// @auth      MGAronya       2022-9-16 12:23
// @param    void
// @return   BeahviorInterface 		返回一个BeahviorInterface用于调用各种函数
func NewGraphTheory() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return GraphTheory{DB: db, Redis: redis, ctx: ctx}
}
