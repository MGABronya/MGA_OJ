// @Title  Search
// @Description  该文件提供关于Search行为的各种方法
// @Author  MGAronya
...
// @return   void
func (a Search) UserBehavior(userId uuid.UUID) (float64, error) {
	var behavior model.Behavior
	if a.DB.Where("name = ? and user_id = ?", "Search", userId).First(&behavior).Error != nil {
		return 0, nil
	}
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Search) PublishBehavior(score float64, userId uuid.UUID) error {
	var behavior model.Behavior
	// TODO 如果没有，直接创建
	if a.DB.Where("name = ? and user_id = ?", "Search", userId).First(&behavior).Error != nil {
	...
	// TODO 更新值
	a.DB.Where("name = ? and user_id = ?", "Search", userId).Save(&behavior)
	var badgeBehaviors []model.BadgeBehavior
	a.DB.Where("name = ?", "Search").Find(&badgeBehaviors)
	// TODO 遍历映射关系，并检查是否更新
...
// @param    ctx *gin.Context       接收一个上下文
// @return   void
func (a Search) Description() string {
	return "用户通过的搜索的题目数量"
}
...
func NewSearch() Interface.BehaviorInterface {
	db := common.GetDB()
	redis := common.GetRedisClient(0)
	ctx := context.Background()
	return Search{DB: db, Redis: redis, ctx: ctx}
}
