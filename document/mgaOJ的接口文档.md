...
- **其它**
  ...
  - **接口地址：/judge/:judge**
    ...
    **方法类型：POST**
    ...
    请求参数：在Body，raw格式给出json类型数据包含 arrs ，其为float的二维数组，表示arrs\[i\]\[j\]表示第i个元素与第j个元素的相似度。指定的最低相似度（在接口地址的judge处，浮点数）。
  
    返回值：成功时，以json格式返回一个二维数组arrs，arrs\[i\]表示第i个连通块中所有的元素，即这些元素互相相似度在最低相似度以上。
