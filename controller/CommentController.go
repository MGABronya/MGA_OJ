...
func (c CommentController) LikeNumber(ctx *gin.Context) {
	...
	// TODO 查看点赞或者点踩的数量
	c.DB.Where("comment_id = (?) and `like` is (?)", id, like).Model(model.CommentLike{}).Count(&total)
...
func (c CommentController) LikeList(ctx *gin.Context) {
	...
	// TODO 查看点赞或者点踩的数量
	c.DB.Where("comment_id = (?) and `like` is (?)", id, like).Order("created_at desc").Offset((pageNum - 1) * pageSize).Limit(pageSize).Find(&commentLikes)
...
func (c CommentController) Likes(ctx *gin.Context) {
	...
	// TODO 查看点赞或者点踩的数量
	c.DB.Where("user_id = (?) and `like` is (?)", id, like).Order("created_at desc").Offset((pageNum - 1) * pageSize).Limit(pageSize).Find(&commentLikes)

	c.DB.Where("user_id = (?) and `like` is (?)", id, like).Model(model.CommentLike{}).Count(&total)
